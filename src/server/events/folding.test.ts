...
describe('digestRound + contextWindows snapshot persistence', () => {
  ...
  it('buildSnapshotFromSessionState omits digestRound but always surfaces contextWindows when no compaction happened', () => {
    const snapshot = buildSnapshotFromSessionState({
      session: {
        mode: 'builder',
        phase: 'plan',
    ...
    expect(snapshot.contextWindows).toEqual([])
  })
