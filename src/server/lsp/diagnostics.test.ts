...
describe('formatDiagnosticsForLLM', () => {
  ...
  it('sorts by severity (errors first)', () => {
    const diagnostics = [
      makeDiagnostic('warning', 'Warning message'),
      makeDiagnostic('error', 'Error message'),
    ]
    const result = formatDiagnosticsForLLM(diagnostics)
  ...
  it('limits output to 10 diagnostics', () => {
    const diagnostics = Array.from({ length: 15 }, (_, i) => 
      makeDiagnostic('error', `Error ${i + 1}`, i)
    )
    const result = formatDiagnosticsForLLM(diagnostics)
  ...
  it('includes correct line numbers (1-indexed for display)', () => {
    const diagnostics = [
      makeDiagnostic('error', 'Error at line 0', 0),
      makeDiagnostic('error', 'Error at line 9', 9),
    ]
    const result = formatDiagnosticsForLLM(diagnostics)
