...
// ============================================================================
// Permission rules merge wiring: disk rules + session-allowed rules
// ============================================================================

describe('runTopLevelAgentLoop permission rules merge', () => {
  let mockEventStore: EventStore
  let mockSessionManager: any
  let mockLLMClient: any
  let mockTurnMetrics: TurnMetrics
  ...
  beforeEach(() => {
    ...
    assembleRequestMock = vi.fn().mockReturnValue({ systemPrompt: 'test', messages: [] })
    ;(getAllInstructions as any).mockResolvedValue({ content: 'test', files: [] })
    ;(getEnabledSkillMetadata as any).mockResolvedValue([])

    mockToolRegistry = {
      tools: [],
      definitions: [],
      execute: vi.fn().mockResolvedValue({ success: true, output: 'ok', durationMs: 0, truncated: false }),
    } as unknown as ToolRegistry

    // Iteration 1: tool batch (captures batchContext), iteration 2: no tools (terminates)
    ;(consumeStreamGenerator as any)
      .mockResolvedValueOnce({
        content: '',
        toolCalls: [{ id: 'call-1', name: 'read_file', arguments: { path: 'a.ts' } }],
    ...
    capturedBatchContext = undefined
  })
  ...
  it('merges disk rules and session-allowed rules into batchContext.permissionRules', async () => {
    // Stub the dynamic imports inside agent-loop
    vi.doMock('../permissions/registry.js', () => ({
      loadMergedRules: vi.fn().mockResolvedValue([{ effect: 'DENY', tool: 'run_command', pattern: 'rm -rf *' }]),
    }))
    vi.doMock('../tools/path-security.js', () => ({
      getSessionAllowedRules: vi.fn().mockReturnValue([{ effect: 'ALLOW', tool: 'read_file', pattern: '/tmp/**' }]),
      clearAllowedPaths: vi.fn(),
      addSessionAllowedRule: vi.fn(),
      addAllowedPath: vi.fn(),
      addAllowedPaths: vi.fn(),
      isPathAllowed: vi.fn().mockReturnValue(false),
      requestPathAccess: vi.fn(),
      PathAccessDeniedError: class PathAccessDeniedError extends Error {},
      AskUserInterrupt: class AskUserInterrupt extends Error {},
    }))

    // Capture batchContext by spying on executeTools
    const executeToolsSpy = vi
      .spyOn(await import('./execute-tools.js'), 'executeTools')
      .mockImplementation(async (_msgId: string, _calls: any, ctx: any) => {
        capturedBatchContext = ctx
        return { toolMessages: [], criteriaChanged: false }
      })

    await runTopLevelAgentLoop(makeConfig(), mockTurnMetrics)

    expect(capturedBatchContext).toBeDefined()
    expect(capturedBatchContext.permissionRules).toEqual([
      { effect: 'DENY', tool: 'run_command', pattern: 'rm -rf *' },
      { effect: 'ALLOW', tool: 'read_file', pattern: '/tmp/**' },
    ])

    executeToolsSpy.mockRestore()
    vi.doUnmock('../permissions/registry.js')
    vi.doUnmock('../tools/path-security.js')
  })

  it('does not set permissionRules when both disk and session rules are empty', async () => {
    vi.doMock('../permissions/registry.js', () => ({
      loadMergedRules: vi.fn().mockResolvedValue([]),
    }))
    vi.doMock('../tools/path-security.js', () => ({
      getSessionAllowedRules: vi.fn().mockReturnValue([]),
      clearAllowedPaths: vi.fn(),
      addSessionAllowedRule: vi.fn(),
      addAllowedPath: vi.fn(),
      addAllowedPaths: vi.fn(),
    ...
    expect(capturedBatchContext).toBeDefined()
    expect(capturedBatchContext.permissionRules).toBeUndefined()
