...
// ============================================================================
// Compaction digest (cumulative compaction summaries)
// ============================================================================

describe('runTopLevelAgentLoop compaction digest', () => {
  let mockEventStore: EventStore
  let mockSessionManager: SessionManager
  let mockLLMClient: any
  let mockTurnMetrics: TurnMetrics
  ...
  function seedEvents(): import('../events/types.js').StoredEvent[] {
    let s = 0
    const ev = (type: string, data: unknown, ts: number) =>
      ({ seq: ++s, timestamp: ts, sessionId: 'test-session', type, data }) as import('../events/types.js').StoredEvent
    return [
      ev('session.initialized', { projectId: 'p', workdir: '/w', contextWindowId: 'window-1' }, 1000),
      ev('message.start', { messageId: 'm1', role: 'user', content: 'hello', contextWindowId: 'window-1' }, 1100),
      ev(
        'message.start',
        {
          messageId: 's1',
      ...
      ev('message.start', { messageId: 'm2', role: 'user', content: 'again', contextWindowId: 'window-2' }, 2100),
      ev(
        'message.start',
        {
          messageId: 's2',
          role: 'assistant',
          content: 'SUMMARY-TWO',
          contextWindowId: 'window-3',
          isCompactionSummary: true,
        },
        3000,
      ),
      ev('message.start', { messageId: 'm3', role: 'user', content: 'more', contextWindowId: 'window-3' }, 3100),
    ]
  }
  ...
  async function runLoopWith(append: ReturnType<typeof vi.fn>, overrides?: Partial<TopLevelLoopConfig>) {
    await runTopLevelAgentLoop(
      {
        mode: 'planner',
        append: append as (event: import('../events/types.js').TurnEvent) => void,
        sessionManager: mockSessionManager,
        sessionId: 'test-session',
        llmClient: mockLLMClient,
        statsIdentity: { providerId: 'test', providerName: 'Test', backend: 'unknown' as const, model: 'test-model' },
  ...
  function appended(append: ReturnType<typeof vi.fn>) {
    return append.mock.calls.map(([event]) => event)
  }

  it('stamps digestRound=0 on context.compacted and emits no digest by default', async () => {
    await runLoopWith(appendMock, { getEvents: vi.fn().mockReturnValue(seedEvents()) })
    const events = appended(appendMock)
    const compacted = events.filter((e: any) => e?.type === 'context.compacted') as any[]
    expect(compacted).toHaveLength(1)
    expect(compacted[0]!.data.digestRound).toBe(0)
    const digests = events.filter(
      (e: any) => e?.type === 'message.start' && (e.data as any)?.metadata?.type === 'compaction-digest',
    )
    expect(digests).toHaveLength(0)
    // Byte-identical LLM context to pre-feature behavior: the emitted event
    // sequence is exactly the current compaction tail — seed relocated, no extra messages.
    expect(events.map((e: any) => e.type)).toEqual(['context.compacted', 'message.start', 'message.done', 'chat.done'])
    const seed = events.find((e: any) => e?.type === 'message.start') as any
    expect(seed.data).toEqual({
      messageId: expect.any(String),
      role: 'assistant',
      content: 'compaction summary',
      contextWindowId: compacted[0]!.data.newWindowId,
      isCompactionSummary: true,
    })
  })

  it('inherits the anti-imitation guard in the rejected-tool-call retry prompt', async () => {
    ;(consumeStreamGenerator as any).mockResolvedValueOnce({
      content: '',
      toolCalls: [{ id: 'call-1', name: 'read_file', arguments: { path: 'foo' } }],
      segments: [],
      usage: { promptTokens: 10, completionTokens: 5 },
      timing: { ttft: 0.1, completionTime: 0.5, tps: 10, prefillTps: 100 },
      aborted: false,
    ...
    const correction = events.find(
      (e: any) => e?.type === 'message.start' && (e.data as any)?.messageKind === 'correction',
    ) as any
    expect(correction).toBeDefined()
    expect(correction.data.content).toContain('STOP and produce a summary for compaction purposes NOW')
    expect(correction.data.content).toContain('do not reproduce, continue, or imitate')
    const digests = events.filter(
      (e: any) => e?.type === 'message.start' && (e.data as any)?.metadata?.type === 'compaction-digest',
    )
    expect(digests).toHaveLength(0)
  ...
  it('emits the digest into the new window before the seed when digestRound=-1', async () => {
    ;(getRuntimeConfig as any).mockReturnValue({
      mode: 'test',
      workdir: '/test',
      agent: { toolTimeout: 60000 },
      context: { compactionThreshold: 0.85, digestRound: -1 },
      llm: {
        baseUrl: 'http://localhost:11434',
        model: 'test-model',
        timeout: 30000,
    ...
    const compacted = events.filter((e: any) => e?.type === 'context.compacted') as any[]
    expect(compacted).toHaveLength(1)
    expect(compacted[0]!.data.digestRound).toBe(-1)
    const newWindowId = compacted[0]!.data.newWindowId

    const starts = events.map((e: any, i: number) => ({ e, i })).filter(({ e }: any) => e?.type === 'message.start')
    const digest = starts.find(({ e }: any) => (e.data as any)?.metadata?.type === 'compaction-digest')
    expect(digest).toBeDefined()
    const digestData = digest!.e.data as any
    expect(digestData.contextWindowId).toBe(newWindowId)
    expect(digestData.metadata.round).toBe(-1)
    expect(digestData.metadata.entries).toEqual([
      { round: 1, windowId: 'window-1', messageId: 's1', summarizedAt: iso(2000) },
      { round: 2, windowId: 'window-2', messageId: 's2', summarizedAt: iso(3000) },
    ])
    expect(digestData.content).toContain('## Round 1 — summarized ' + iso(2000))
    expect(digestData.content).toContain('## Round 2 — summarized ' + iso(3000))
    expect(digestData.content).toContain('SUMMARY-ONE')
    expect(digestData.content).toContain('SUMMARY-TWO')
    expect(digestData.content).toContain('compaction summary of Round 3')

    const seed = starts.find(({ e }: any) => (e.data as any)?.isCompactionSummary === true)
    expect(seed).toBeDefined()
    expect(digest!.i).toBeLessThan(seed!.i)

    const digestDone = events.filter(
      (e: any) => e?.type === 'message.done' && e.data.messageId === digestData.messageId,
    )
    expect(digestDone).toHaveLength(1)
  })

  it('caps the digest at k rounds when digestRound=k', async () => {
    ;(getRuntimeConfig as any).mockReturnValue({
      mode: 'test',
      workdir: '/test',
      agent: { toolTimeout: 60000 },
    ...
    expect(digest).toBeDefined()
    expect(digest.data.metadata.round).toBe(1)
    expect(digest.data.metadata.entries).toHaveLength(1)
    expect(digest.data.metadata.entries[0]).toMatchObject({ round: 2, messageId: 's2' })
    expect(digest.data.content).toContain('SUMMARY-TWO')
    expect(digest.data.content).not.toContain('SUMMARY-ONE')
    expect(digest.data.content).toContain('most recent 1 round summaries are included below')
  })

  it('emits no digest for the first compaction (no prior rounds) even with -1', async () => {
    ;(getRuntimeConfig as any).mockReturnValue({
      mode: 'test',
      workdir: '/test',
      agent: { toolTimeout: 60000 },
    ...
    vi.clearAllMocks()
    const firstWindowEvents = seedEvents().slice(0, 2)
    await runLoopWith(appendMock, { getEvents: vi.fn().mockReturnValue(firstWindowEvents) })
  ...
  it('emits no digest for sub-agent scopes (top-level only) but still stamps the decision', async () => {
    ;(getRuntimeConfig as any).mockReturnValue({
      mode: 'test',
      workdir: '/test',
      agent: { toolTimeout: 60000 },
    ...
    vi.clearAllMocks()
    await runLoopWith(appendMock, {
      subAgentMetadata: { subAgentId: 'explorer-1', subAgentType: 'explorer' },
      getEvents: vi.fn().mockReturnValue(seedEvents()),
    })
