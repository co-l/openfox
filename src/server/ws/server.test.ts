...
async function createHarness(
  ...
  const wss = createWebSocketServer(
    ...
    undefined,
    options.getTransitionHandlers as never,
  )
...
describe('createWebSocketServer', () => {
  ...
  it('forwards transitionHandlers from getTransitionHandlers to runOrchestrator on runner.launch', async () => {
    const { TransitionHandlerRegistry } = await import('../workflows/transition-handlers.js')
    ...
    const harness = await createHarness({ sessionManager, getTransitionHandlers: () => registry })

    harness.send({ id: 'sl-ok', type: 'session.load', payload: { sessionId: 'session-1' } })
    await harness.nextMessage((message) => message.id === 'sl-ok')

    harness.send({ id: 'runner-th', type: 'runner.launch', payload: { workflowId: 'dream' } })
    await harness.nextMessage((message) => message.id === 'runner-th')
    await new Promise<void>((resolve) => setTimeout(resolve, 0))

    expect(runOrchestratorMock).toHaveBeenCalled()
    const callArgs = runOrchestratorMock.mock.calls[0]![0]
    expect(callArgs.transitionHandlers).toBe(registry)
  })
