...
import { providePathConfirmation, addAllowedPaths, cancelQuestionsForSession, cancelPathConfirmationsForSession, } from '../tools/index.js';
...
import { parseClientMessage, serializeServerMessage, createErrorMessage, createSessionStateMessage, createSessionListMessage, createSessionRunningMessage, createProjectStateMessage, createProjectListMessage, createChatDeltaMessage, createChatThinkingMessage, createChatToolCallMessage, createChatToolResultMessage, createChatMessageMessage, createChatMessageUpdatedMessage, createChatDoneMessage, createChatErrorMessage, createModeChangedMessage, createPhaseChangedMessage, createCriteriaUpdatedMessage, createContextStateMessage, isProjectCreatePayload, isProjectCreateWithDirPayload, isProjectLoadPayload, isProjectUpdatePayload, isProjectDeletePayload, isSessionCreatePayload, isSessionLoadPayload, isChatSendPayload, isModeSwitchPayload, isCriteriaEditPayload, isPathConfirmPayload, isSettingsGetPayload, isSettingsSetPayload, isSessionSetProviderPayload, createSettingsValueMessage, storedEventToServerMessage, createQueueStateMessage, isQueueAsapPayload, isQueueCompletionPayload, isQueueCancelPayload, } from './protocol.js';
...
export function createWebSocketServer(httpServer, config, getLLMClient, getActiveProvider, toolRegistry, sessionManager, providerManager) {
    const wss = new WebSocketServer({ server: httpServer, path: '/ws' });
    const clients = new Map();
    // Per-session LLM client cache: sessionId -> { cacheKey, client }
    const sessionLLMClients = new Map();
    ...
    wss.on('connection', (ws) => {
        ...
        ws.on('message', async (data) => {
            ...
            try {
                ...
                ws.send(serializeServerMessage(createErrorMessage('INTERNAL_ERROR', error instanceof Error ? error.message : 'Unknown error', message.id)));
            }
        });
        ws.on('close', () => {
...
async function handleClientMessage(ws, client, message, config, getLLMClient, getActiveProvider, toolRegistry, sessionManager, broadcastForSession, providerManager, getSessionLLMClient, getSessionStatsIdentity, invalidateSessionLLMClient) {
    const send = (msg) => {
    ...
    switch (message.type) {
        ...
        case 'session.create': {
            ...
            const session = sessionManager.createSession(message.payload.projectId, message.payload.title, currentProvider?.id ?? null, currentModel);
        ...
        case 'context.compact': {
            ...
            (async () => {
                try {
                    ...
                    sendForSession(sessionId, createChatErrorMessage(`Compaction failed: ${error instanceof Error ? error.message : 'Unknown error'}`, true));
                }
            })();
            break;
        }
