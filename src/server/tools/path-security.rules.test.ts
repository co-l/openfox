...
describe('requestPathAccess with permission rules', () => {
  ...
  it('ALLOW on one path does not exempt the other paths of the same call', async () => {
    const rules = [rule('ALLOW', 'read_file', '/ubiquity/**')]
    const promise = requestPathAccess(
      ['/ubiquity/deploy.yaml', OUTSIDE + '/other.txt'],
      WORKDIR,
      'rules-session',
      'c20',
      'read_file',
  ...
  it('ALLOW on every path of the call skips the sandbox check entirely', async () => {
    const rules = [rule('ALLOW', 'read_file', '/ubiquity/**')]
    await expect(
      requestPathAccess(
        ['/ubiquity/a.yaml', '/ubiquity/b.yaml'],
        WORKDIR,
        'rules-session',
        'c21',
        'read_file',
    ...
    expect(noOpEvent).not.toHaveBeenCalled()
    expect(hasPendingPathConfirmation('c21')).toBe(false)
  })

  it('ALLOW on the run_command target covers the whole call', async () => {
    const rules = [rule('ALLOW', 'run_command', 'terraform plan *')]
    await expect(
      requestPathAccess(
        [OUTSIDE + '/state.tf'],
        WORKDIR,
    ...
    expect(hasPendingPathConfirmation('c22')).toBe(false)
  })

  it('DENY on one path still blocks a call whose other path is ALLOWed', async () => {
    const rules = [rule('ALLOW', 'read_file', '/ubiquity/**'), rule('DENY', 'read_file', '/secret/**')]
    await expect(
      requestPathAccess(
        ['/ubiquity/deploy.yaml', '/secret/key.pem'],
        WORKDIR,
        'rules-session',
        'c23',
        'read_file',
    ...
    ).rejects.toMatchObject({ reason: 'rule_denied' })
    expect(hasPendingPathConfirmation('c23')).toBe(false)
  })
})
