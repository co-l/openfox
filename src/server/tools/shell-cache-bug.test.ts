...
describe('shell tool cache preservation bug', () => {
  ...
  it('should return consistent output for the same failing command across multiple calls', async () => {
    ...
    // First call
    const result1 = await runCommandTool.execute({ command }, context)
    ...
    const result2 = await runCommandTool.execute({ command }, context)
...
`,
    ...
    const result = await runCommandTool.execute({ command: `bash ${scriptPath}` }, context)
