...
vi.mock('./path-security.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    requestPathAccess: vi
      .fn()
      .mockRejectedValue(new actual.PathAccessDeniedError(['/test/.env'], 'test_tool', 'sensitive_file')),
  }
})
...
describe('PathAccessDeniedError propagation', () => {
  ...
  describe('read_file', () => {
    it('re-throws PathAccessDeniedError instead of catching it', async () => {
      ...
      await expect(readFileTool.execute({ path: '.env' }, context)).rejects.toThrow(PathAccessDeniedError)
    })
  })

  describe('write_file', () => {
    it('re-throws PathAccessDeniedError instead of catching it', async () => {
      await expect(writeFileTool.execute({ path: '.env', content: 'test' }, context)).rejects.toThrow(
  ...
  describe('edit_file', () => {
    it('re-throws PathAccessDeniedError instead of catching it', async () => {
      await expect(editFileTool.execute({ path: '.env', old_string: 'a', new_string: 'b' }, context)).rejects.toThrow(
        PathAccessDeniedError,
      )
    })
  })
  ...
  describe('run_command', () => {
    it('re-throws PathAccessDeniedError instead of catching it', async () => {
      await expect(runCommandTool.execute({ command: 'cat .env' }, context)).rejects.toThrow(PathAccessDeniedError)
    })
  })
})
