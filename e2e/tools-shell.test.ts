  ...
  describe('Output Streaming', () => {
    it('streams stdout before tool_result', async () => {
      ...
      const outputEvents = allEvents.filter(
        (e) => e.type === 'chat.tool_output' && (e.payload as { callId: string }).callId === runCall!.callId,
      )
      ...
      const resultEvent = allEvents.find(
        (e) => e.type === 'chat.tool_result' && (e.payload as { callId: string }).callId === runCall!.callId,
      )
    ...
    it('streams stderr separately', async () => {
      ...
      const stdoutEvents = outputEvents.filter((e) => (e.payload as { stream: string }).stream === 'stdout')
      const stderrEvents = outputEvents.filter((e) => (e.payload as { stream: string }).stream === 'stderr')

      expect(stdoutEvents.length).toBeGreaterThan(0)
      expect(stderrEvents.length).toBeGreaterThan(0)

      // Verify content
      const stdoutContent = stdoutEvents.map((e) => (e.payload as { output: string }).output).join('')
      const stderrContent = stderrEvents.map((e) => (e.payload as { output: string }).output).join('')
    ...
    it('streams output for slow commands', async () => {
      ...
      const allOutput = outputEvents.map((e) => (e.payload as { output: string }).output).join('')
