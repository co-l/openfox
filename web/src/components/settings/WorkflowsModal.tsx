...
function computeLayout(steps: WorkflowStep[], entryStep: string) {
    ...
    <div className="space-y-3 text-sm">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <select value={step.type} onChange={e => update('type', e.target.value as WorkflowStep['type'])} className={selectClass}>
...
export function WorkflowsModal({ isOpen, onClose }: WorkflowsModalProps) {
  ...
  const handleEdit = async (workflowId: string) => {
    ...
    setFormMaxVerifyRetries(workflow.settings.maxVerifyRetries)
  ...
  const handleSave = async () => {
    ...
    if (formSteps.length === 0) { setFormError('Workflow must have at least one step.'); return }
  ...
  const addStep = () => {
    const newStep: WorkflowStep = { id: '', name: '', type: 'llm_turn', phase: 'build', toolMode: 'builder', transitions: [] }
          ...
          {workflows.map(workflow => (
            <div key={workflow.id} className="flex items-center justify-between p-3 rounded border border-border bg-bg-tertiary">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <button onClick={() => handleActivate(workflow.id)} className="px-2 py-1 rounded text-xs bg-accent-primary/10 text-accent-primary hover:bg-accent-primary/20 transition-colors" title="Set as active">Activate</button>
                <button onClick={() => handleEdit(workflow.id)} className="p-1.5 rounded hover:bg-bg-primary text-text-muted hover:text-text-primary transition-colors" title="Edit">
                ...
                {workflow.id !== 'default' && (
                  confirmDeleteId === workflow.id ? (
                    ...
                    <button onClick={() => setConfirmDeleteId(workflow.id)} className="p-1.5 rounded hover:bg-bg-primary text-text-muted hover:text-accent-error transition-colors" title="Delete">
