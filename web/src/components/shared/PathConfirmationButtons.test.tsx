...
describe('PathConfirmationButtons', () => {
  ...
  it('"Allow Everything" switches dangerous mode and lets the server auto-approve', async () => {
    switchDangerLevelMock.mockResolvedValueOnce(true)
    const container = renderConfirmation('rule_ask')
    ...
    expect(confirmPathMock).not.toHaveBeenCalled()
  })

  it('"Allow Everything" falls back to approving this confirmation when the switch fails', async () => {
    switchDangerLevelMock.mockResolvedValueOnce(false)
    const container = renderConfirmation('rule_ask')
    const btn = getButtonByText(container, 'Allow Everything')!
    await act(async () => {
      btn.dispatchEvent(new MouseEvent('click', { bubbles: true }))
