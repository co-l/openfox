...
export const TodoListDisplay = memo(function TodoListDisplay({ todos }: TodoListDisplayProps) {
  ...
  if (!Array.isArray(todos)) {
    return <div className="text-xs text-accent-warning italic my-1">⚠️ Invalid todos data</div>
  }

  if (todos.length === 0) {
    return <div className="text-xs text-text-muted italic my-1">No tasks</div>
  }
