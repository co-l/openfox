  ...
  if (!Array.isArray(todos)) {
    return (
      <div className="text-xs text-accent-warning italic my-1">
        {t({ en: '⚠️ Invalid todos data', fr: '⚠️ Données de tâches invalides' })}
      </div>
    )
  }

  if (todos.length === 0) {
    return <div className="text-xs text-text-muted italic my-1">{t({ en: 'No tasks', fr: 'Aucune tâche' })}</div>
  }
  ...
  return (
    <div className="my-1 rounded border border-border bg-bg-tertiary overflow-hidden">
      ...
      <div className="px-2 py-1.5 border-b border-border bg-bg-secondary">
        <span className="text-xs font-medium text-text-muted">{t({ en: 'Tasks', fr: 'Tâches' })}</span>
      </div>
      ...
      <div>
        {todos.map((todo, index) => {
          ...
          return (
            ...
            >
              <span
                className={`${config?.color ?? 'text-text-muted'} ${config?.animate ? 'animate-pulse' : ''} text-sm leading-tight`}
              >
                {config?.icon ?? '○'}
              </span>
              <span
