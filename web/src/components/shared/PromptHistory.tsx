      ...
      {history.map((item, index) => (
        <div
          ...
          className={`
            ...
            ${index === selectedIndex ? 'bg-accent-primary/20 border border-accent-primary' : 'hover:bg-bg-tertiary'}
          `}
        >
          <div className="text-xs text-text-muted font-mono">
            ...
            {item.sessionName && (
              <>
                {' | '}
                <span className="text-accent-primary font-medium">{item.sessionName}</span>
              </>
            )}
          </div>
