    ...
    <div className="flex-1 flex flex-col overflow-y-auto">
      <div className="max-w-5xl mx-auto w-full p-4 md:p-8">
        ...
        {sortedProjects.length > 0 && (
          <div className="mb-6 md:mb-8">
            <h2 className="text-lg font-semibold text-text-primary mb-3">Projects</h2>

            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {sortedProjects.map((project) => (
                <div
                  key={project.id}
                  className="bg-bg-secondary border border-border rounded-lg p-4 hover:bg-bg-tertiary hover:border-accent-primary transition-colors group relative cursor-pointer"
                ...
                >
                  <div className="flex items-center gap-2 relative z-10">
                    ...
                    <span className="text-text-primary font-medium truncate flex-1 text-left">{project.name}</span>
                  </div>
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                    ...
                    className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-accent-error/70 hover:text-accent-error p-1 transition-opacity z-20"
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded flex items-center gap-1 flex-shrink-0 ${getPhaseBadgeClasses(session.phase)}`}
                        >
                          {session.phase === 'done' ? (
                            <>
        ...
        {/* Primary CTA */}
        <div>
          <Button variant="primary" className="w-full py-4 text-lg font-semibold" onClick={handleOpenProject}>
            Open Project
          </Button>
        </div>
      </div>
